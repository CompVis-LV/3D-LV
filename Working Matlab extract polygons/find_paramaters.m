...
RGB = '6_maskedImage.png';
RGB = imread(RGB);
img = rgb2gray(RGB);
a = (0.05).*rand(1,1);
b = (1-0.7).*rand(1,1) + 0.7;
img = imadjust(img,[a b],[]); % I is double
%img = imfilter(img, ones(4)/16);
k = ((0.5-0).*rand(1,1)+0)
imgB = imgaussfilt(img, k);
c = ((5).*rand(1,1));
d = ((5).*rand(1,1));
imgC = imdiffusefilt(imgB,'GradientThreshold', [c, d],'NumberOfIterations',2);
e = ((3-0.1).*rand(1,1)+0.1);
f = ((3-0.1).*rand(1,1)+0.1);
imgD = imsharpen(imgC,'Radius',e,'Amount',f);
g = ((10-0.1).*rand(1,1)+0.1);
h = ((3-0.1).*rand(1,1)+0.1);
i = round((30-10).*rand(1,1)+10)
[lines, image] = haughTransformWORKING(imgD, g, h, i, 10, 70);
j = ((60-30).*rand(1,1)+30);
linesJoined = joinVertices(lines, j);
...
%%
addpath(genpath('C:\Users\Jared\Documents\ECTE458\3D-LV'))
cd C:\Users\Jared\Documents\ECTE458\3D-LV\Datasets\pyramidcopy
RGB = '12_maskedImage.png';
RGB = imread(RGB);
img = rgb2gray(RGB);
img = imadjust(img,[a b],[]); % I is double
%img = imfilter(img, ones(4)/16);
imgB = imgaussfilt(img, (0.3));
imgC = imdiffusefilt(imgB,'GradientThreshold', [c, d],'NumberOfIterations',2);
imgD = imsharpen(imgC,'Radius',e,'Amount',f);
[lines, image] = haughTransformWORKING(imgD, g, h, i, 10, 70);
linesJoined = joinVertices(lines, j);
...
count = 0;
while (startPoint ~= 0 & count < 10000)
